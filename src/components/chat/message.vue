  ...
  <div v-show="activedKey[type]!= ''" class="messagebox">
        ...
        >
          <span style="user-select: none">
            ...
            <div v-else-if="item.type==='audio'" :style="{'float':item.bySelf ? 'right':'left'}">
              <audio :src="item.msg" controls />
            </div>
            <!-- 视频消息 -->
            <div v-else-if="item.type==='video'">
              <video :src="item.msg" width="100%" controls />
            </div>
            <!-- 聊天消息 -->
            <p
              v-else
    ...
    <div class="messagebox-footer">
      <div class="footer-icon">
        ...
        <!-- 上传图片组件 -->
        <UpLoadImage :type="type" :chat-id="activedKey[type]" />
        <!-- 上传文件组件 -->
        <UpLoadFile :type="type" :chat-id="activedKey[type]" />
