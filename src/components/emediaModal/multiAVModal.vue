<template>
  <Draggable v-if="multiAVModalVisible" id="drag3">
    <div v-show="multiAVModalVisible" class="multiAVModal">
      <span class="title">{{ to.groupname }}</span>
      <!-- <div>时间</div> -->
      <el-row type="flex" class="row-bg" justify="space-around">
        <el-col :span="6"><div class="grid-content bg-purple videoBox">
          <video ref="rv_local" class="video" autoPlay playsInline muted />
          <div class="tools">
            <span>{{ rv_local.nickName }}</span>
          </div>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple-light videoBox">
          <video ref="rv_0" class="video" autoPlay playsInline />
          <div class="tools">
            <span>{{ rv[0].nickName }}</span>
            <i class="el-icon-video-camera red" @click="controlRemoteVideo(0)" />
          </div>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple videoBox">
          <video ref="rv_1" class="video" autoPlay playsInline />
          <div class="tools">
            <span>{{ rv[1].nickName }}</span>
            <i class="el-icon-video-camera red" @click="controlRemoteVideo(1)" />
          </div>
        </div></el-col>
      </el-row>
      <el-row type="flex" class="row-bg" justify="space-around">
        <el-col :span="6"><div class="grid-content bg-purple videoBox">
          <video ref="rv_2" class="video" autoPlay playsInline />
          <div class="tools">
            <span>{{ rv[2].nickName }}</span>
            <i class="el-icon-video-camera red" @click="controlRemoteVideo(2)" />
          </div>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple-light videoBox">
          <video ref="rv_3" class="video" autoPlay playsInline />
          <div class="tools">
            <span>{{ rv[3].nickName }}</span>
            <i class="el-icon-video-camera red" @click="controlRemoteVideo(3)" />
          </div>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple videoBox">
          <video ref="rv_4" class="video" autoPlay playsInline />
          <div class="tools">
            <span>{{ rv[4].nickName }}</span>
            <i class="el-icon-video-camera red" @click="controlRemoteVideo(4)" />
          </div>
        </div></el-col>
      </el-row>
      <div class="toolsBox">
        <el-col :span="3"><i class="el-icon-circle-plus-outline toolBtn" @click="invite" /></el-col>
        <!-- <el-col :span="3"><i class="el-icon-phone-outline toolBtn"></i></el-col> -->
        <el-col :span="3"><i :class="rv_local.openAudio?'el-icon-microphone toolBtn':'el-icon-microphone active'" @click="controlLocalMic" /></el-col>
        <el-col :span="3"><i :class="rv_local.openVideo?'el-icon-video-camera toolBtn':'el-icon-video-camera active'" @click="controlLocalVideo" /></el-col>
  ...
  methods: {
    ...
    initEmedia() {
      ...
      WebIM.EMService.onStreamAdded = function(member, stream) {
        ...
        let index
        if (located) {
          const localVideo = me.$refs.rv_local
          ...
          const rv = me.$data.rv; let rvCount = me.$data.rvCount
          const nickName = member.name
    ...
    controlRemoteVideo(id) {
      ...
      const rv = this.$data.rv
      const elem = rv[id]
      if (elem.streamId === '') {
        return
      }
      const video = this.$refs['rv_' + id]
      if (elem.openVideo) {
    ...
    controlLocalVideo() {
      const localVideo = this.$refs.rv_local
      const { stream, localStreamId, openAudio, openVideo } = this.$data.rv_local
      if (openVideo) {
