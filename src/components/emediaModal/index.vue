<template>
  <Draggable v-show="emediaModalVisible" id="drag1">
    <div :class="{rtcVoiceContent: streamType=='语音', rtcVideoContent: streamType=='视频'}">
      ...
      <video v-show="streamType == '视频'" ref="localVideo" :class="{localVideo: toggle, remoteVideo: !toggle}" autoPlay muted playsInline />
      <video v-show="streamType == '视频'" ref="remoteVideo" :class="{localVideo: !toggle, remoteVideo: toggle}" autoPlay playsInline />
      <i v-show="showMute" ref="audio" class="el-icon-turn-off-microphone font microphone" isopen="true" @click="controlStream('audioControl')" />
      <i v-show="showAccept" class="el-icon-phone font accept" isopen="true" @click="accept" />
      ...
      <a-icon v-show="showMute && streamType=='视频'" ref="video" class="font camera" isopen="true" type="video-camera" @click="controlStream('videoControl')" />
      <!-- <i v-show="showMute" class="el-icon-headset font mute" ref="mute" @click="mute"></i> -->
      <a-icon v-show="showMute" ref="mute" class="font mute" type="sound" @click="mute" />
      <i class="el-icon-switch-button close" @click="close" />
      <i v-show="showMute && streamType=='视频'" class="el-icon-refresh font toggle" @click="toggleClick" />
    </div>
  </Draggable>
</template>
...
export default {
  ...
  methods: {
    ...
    controlStream(type) {
      let controlType
      const to = WebIM.call.callee.split('@')[0].split('_')[1]
