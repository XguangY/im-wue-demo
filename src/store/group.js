...
const Group = {
  ...
  actions: {
    ...
    onGetGroupinfo: function(context, payload) {
      ...
      // TODO
      const gid = payload.select_id || payload.select_groupid
      const options = {
        groupId: gid, // 群组id
        success: function(resp) {
          const name = resp.data[0].name
          const admin = resp.data[0].owner
          const desc = resp.data[0].description
          const membersonly = resp.data[0].membersonly
          const members = resp.data[0].affiliations
    ...
    onGetGroupBlack: function(context, payload) {
      const select_id = payload.groupid || payload.select_id
      const option = {
        groupId: select_id,
        success: function(list) {
          const blackName = list.data
